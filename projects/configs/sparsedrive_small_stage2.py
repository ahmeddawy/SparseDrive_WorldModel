...
optimizer = dict(
    type="AdamW",
    lr=5e-5,  # modest bump to compensate for small batch
    weight_decay=0.01,
    paramwise_cfg=dict(
        custom_keys={
            "img_backbone": dict(lr_mult=0.1),
            "head.det_head": dict(lr_mult=0.5),
            "head.map_head": dict(lr_mult=0.5),
            "head.motion_plan_head": dict(lr_mult=0.8),
...
optimizer_config = dict(
    grad_clip=dict(max_norm=25.0, norm_type=2),  # Keep your gradient clipping
)
